...
5. **驗證量能門檻出現**：下次個股分析確認報表「等待條件」有具體張數（如「突破需 1,800 張」）
6. **驗證 P&F 目標合理性**：若顯示「—（尚未接近突破點）」代表修法生效
7. **K線型態歷史累積**：目前第一批資料已入 DB，待 1-2 個月後查看 return_3/5/10d 回填結果
8. 清理遺留
   - 還原時間鎖 `< 0` → `< 15`、冷卻 `< 0` → `< 4 * 3600`（`app.py`）
   - 移除「🗑 清快取」按鈕（`dashboard.html`、`dashboard.js`、`app.py`）
9. 重新跑一次週報驗證 commit `c95d0dd` 修法（會燒 ~$0.20）
10. （未做）「移除聯絡人」UI — v1 沒做，要刪要進 DB
