...
## 當前進度（2026-05-08 四項修復完成）

**所在週次：週4（進行中）**

**狀態：HEAD = `9ef2628`（已 push）**
...
**下一步（按優先順序）：**
1. **觀察今日 14:30 批次**：確認財經新聞三個區塊（重大財經重點 + 隔日方向注意 + 操作建議）正確產生
2. **驗證量能門檻出現**：下次個股分析確認報表「等待條件」有具體張數（如「突破需 1,800 張」）
3. **驗證 P&F 目標合理性**：若顯示「—（尚未接近突破點）」代表修法生效；若仍偏高觀察 base_high 是否合理
4. **K線型態歷史累積**：目前第一批資料已入 DB，待 1-2 個月後查看 return_3/5/10d 回填結果
5. 清理遺留
   - 還原時間鎖 `< 0` → `< 15`、冷卻 `< 0` → `< 4 * 3600`（`app.py`）
   - 移除「🗑 清快取」按鈕（`dashboard.html`、`dashboard.js`、`app.py`）
6. 重新跑一次週報驗證 commit `c95d0dd` 修法（會燒 ~$0.20）
7. （未做）「移除聯絡人」UI — v1 沒做，要刪要進 DB
