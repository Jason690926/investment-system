...
## 當前進度（2026-05-11 K線型態全面Bug修正）

**所在週次：週7**

**狀態：HEAD = `5b0fd26`（已 push）**
...
**下一步（按優先順序）：**
1. **驗證三山頂修正**：觀察原本天天顯示三山頂的個股，確認 15 根靜音機制生效
2. **驗證早晨之星/黃昏之星**：確認小陰+小實體+小陽不再誤觸發（需 3 根實體均 > 50% 振幅才觸發）
3. **驗證週末視窗**：週五 14:00 後按一鍵分析，確認按鈕顯示「含週報」、週報背景產生、PDF 顯示 §MARKET
4. **驗證量能門檻出現**：下次個股分析確認報表「等待條件」有具體張數（如「突破需 1,800 張」）
5. **驗證 P&F 目標合理性**：若顯示「—（尚未接近突破點）」代表修法生效
6. **K線型態歷史累積**：目前第一批資料已入 DB，待 1-2 個月後查看 return_3/5/10d 回填結果
7. 清理遺留
   - 還原時間鎖 `< 0` → `< 15`、冷卻 `< 0` → `< 4 * 3600`（`app.py`）
   - 移除「🗑 清快取」按鈕（`dashboard.html`、`dashboard.js`、`app.py`）
8. 重新跑一次週報驗證 commit `c95d0dd` 修法（會燒 ~$0.20）
9. （未做）「移除聯絡人」UI — v1 沒做，要刪要進 DB
