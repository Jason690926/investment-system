...
## §三十八 強漲回測股誠實揭露 + 看板即時離區（2026-06-09）

### 緣起
用戶 6/9 收盤截圖 cross-check 6/8 報表：合晶/矽力等「5 月強漲 → 6 月回測」觀察股，
進場區是 `calc_swing_levels` 大箱（合晶 52.1–76.8 寬 31%、矽力 391–542 寬 29%），
價已脫離或停損距現價 -25% 以上，且 P&F「已達成」→ 標「—」，對家人讀者不可操作。
根因：§三十二 `_breakout_overrides` 只在「新鮮突破」縮窄區間，強漲回測股不觸發。

### 決策（brainstorming）
- **誠實揭露**（不硬縮區間/不湊目標），保留客觀失效價
- **gate = 區間過寬（(entry_high−entry_low)÷price > 25%）**；①脫離原箱僅作 strip 標籤
  （①單獨「價>上緣」會誤殺窄區間正常等回測；分析時價>上緣必伴隨寬區間 → 收斂為②為 gate）
- pill 統一 `🟡 強漲回測觀望`；**僅 long**；零 DB migration

### 兩塊分工
- **#2 後端分析時**：價脫離/區過寬 → pill + 第五節烤入（PDF 第五節 + 建議 pill 經
  `[[OPERATION_FRAMEWORK]]` 注入 + `action_pill` DB 欄位**自動同步**，app.py 零改）
- **#1 前端看板時**：分析當下在區內、之後即時價漂出 → strip 變灰 + ↑/↓ 微標
  （pill 為 🟢 actionable 且非追進才標，避免噪音）

### 6 commit（TDD，pytest 358→375 全綠）
| commit | 內容 |
|--------|------|
| `f0e3bd2` test+feat | `_strong_pullback_state` 純函式（區間過寬偵測，8 case）|
| `1b8ebc8` feat | `_decide_action` WATCH long 插 gate → 強漲回測觀望（跌穿優先、breakout 不誤觸，6 case）|
| `12b56ad` feat | `_render_operation_framework` 誠實第五節 + `price` 參數 + 兩 call site（3 case）|
| `4283920` feat | 看板 `renderAnchorStrip` #2 誠實 strip + #1 即時離區灰標 + `updateCardPrice` 重繪 |
| `b75bea0` feat | app.css `.drift-out` 灰 + `.anchor-drift-tag` 琥珀微標 |
| 本 commit | docs plan.md §三十八 |

### 驗收（重跑後）
- 合晶/矽力標頭 pill = `🟡 強漲回測觀望`；第五節無假進場區/「—」目標，只剩失效價 + 待新箱
- 看板（零 token，即時生效）：微星/創惟即時價漂出進場區 → strip 變灰 + ↑ 價已離區

### 回滾
6 commit 純加性各自獨立 revert；`_render_operation_framework` 新增 `price=None` 為 optional；
無 migration、無破壞性簽名改動。
...
## §四十 7/13 報表 cross-check：假棒污染 + 空停零距離佈空 + 突破目標雙源（2026-07-13）

### 緣起
用戶 7/13 報告（12 檔）審視發現 1 資料層 bug + 2 邏輯矛盾：

1. **7/10 假 K 棒污染全 12 檔**：Yahoo 對休市/停牌日回「O=H=L=C=前收、Volume=null」
   佔位棒。PDF K 表印出字面 "None"、假棒被標「縮量·極低位」特徵餵 AI、
   5 日均量被拉低 → 放量判定與突破量門檻（×1.5）全失真（晶心科 5日均 264 張含 0 量假棒）。
   §二十六 Bug5 只修「null Close 補 (H+L)/2」，沒擋整根退化棒。
2. **「分批佈空」在空停零距離觸發**：short `entry_zone=(mid, swing_high)`、
   `invalidation=swing_high` → 價在區頂 = 停損零距離。晶心科現價 210.5＝空停 210.5
   （且當日一字漲停鎖死，實務不可空）仍標 🔴 分批佈空；采鈺距空停僅 1.31%。
   §三十四 Bug-3 只在區**底**加 0.5% buffer，區**頂**無對稱防護。
3. **強勢突破股目標價雙源矛盾**：`_dual_pnf`（prompt 注入，AI 呼叫**前**）用標準
   `calc_pnf_target`；`_breakout_overrides`（post-process，AI 呼叫**後**）用 60 日低重算。
   → 大聯大第四節「P&F概念目標：120元」vs 第五節/pill「目標 133.00」；微星 159 vs 196.2。
   §三十四 S2「pill 與內文同源」沒覆蓋 breakout override 路徑。

### 修法（3 fix，全純函式/資料層，零 migration、零新 prompt 行為）

| Fix | 修法 |
|-----|------|
| **F1 假棒剔除** | `data_enricher._drop_degenerate_bars`：`(Volume isna 或 ==0) 且 O==H==L==C` → 整列 drop；掛在 `_yahoo_ohlcv` 內 `_patch_missing_close` 之後（日/週/月全週期共用）。真一字漲停 O=H=L=C 但量>0 不受影響 |
| **F2 空停距離防護** | `_decide_action` short 在區內分支加雙 gate：(a) `price ≥ 空停參考價（invalidation，缺則 zone 頂）× 0.98`（距空停 <2%）或 (b) 今日一字漲停鎖死（新純函式 `_limit_up_locked_today`，漲幅≥9% + close≥high×0.99）→ 降級 `🟡 等反轉佈空`（取代 🔴 分批佈空）。pill class 走 emoji 前綴，前端零改 |
| **F3 目標同源** | `_dual_pnf` 加 `breakout=False` 參數：成立時 long 句改用 `_breakout_overrides` target（與第五節/pill 同源）；兩 call site 先算 `_strong_breakout_state` 再傳入（market_only 原本就有 pre-prompt 計算，改共用變數） |

### 設計決策
1. **F1 濾在資料層而非 _fmt_bars**：假棒同時污染 表格/特徵標注/均量/型態偵測/MA，
   源頭 drop 一次全修；只砍「量缺 + 全平」交集，流動性差但有量的真實棒保留。
2. **F2 門檻 2%**：采鈺實例 1.31% 須觸發；空單停損距離 <2% R/R 不成立；
   區底 buffer 0.5%（§三十四）語意不同（是否已入區）故不對稱。
3. **F2 一字漲停抑制**：漲停鎖死日券源/借券實務不可空、隔日慣性風險高；
   偵測邏輯與 `_strong_breakout_state` 條件 C 內核一致，抽純函式共用語意。
4. **F3 在 prompt 注入時就同源**（而非 post-process regex 改 AI 輸出）：
   `_strong_breakout_state`/`_breakout_overrides` 均為 enriched_data 純函式，
   pre/post 計算結果 deterministic 一致；AI verbatim 引用即自然同源。

### 驗收（燒 ~$0.6 重跑後）
- K 表不再出現 "None" 假棒列；5 日均量不含 0 量日
- 晶心科/采鈺類「價貼空停」場景 pill = 🟡 等反轉佈空；漲停日不再建議佈空
- 大聯大/微星類強勢突破股：第四節 P&F 句 = 第五節目標 = pill 同值
...
## §四十五 P&F 句方向錯配強制矯正（2026-07-17）

### 緣起（7/17 20:35 報告 cross-check）
`_dual_pnf` 注入多方句/空方句/neutral 指示三選一，由 AI 依 DIRECTION 挑 — 抓到兩檔挑錯：
- **撼訊**（DIRECTION=short，價 116）：§4 引多方失效句「論點已失效（跌穿支撐 121 元）」
- **晶心科**（DIRECTION=neutral）：§4 引 long relaxed 句「理論目標 176 — 已跌破 213」
  （應為「—（無方向，待結構確認）」）

### 修法
1. `_pnf_sentences(enriched_data, price_f, breakout)` — 自 `_dual_pnf` 抽出句子計算
   （`_dual_pnf` 改薄 wrapper，回傳簽名不變、既有測試零影響）；prompt 注入（AI 前）
   與 post-process 矯正（AI 後）共用 → 同源保證
2. `_enforce_pnf_direction(html, direction, long_s, short_s)` — regex
   `P&F\s*(?:概念|理論)\s*目\s*標\s*：[^<\n]*` 掃 AI html，與 direction 對應句不符
   → 全部替換；找不到句子（AI 漏寫）→ 原樣不亂插；neutral → 固定句
3. 兩 call site 於 §三十一 post-process（`action_pill` 之後、framework 注入之前）
   接線 — 用**最終 direction**（含 §三十二 safety net 覆寫後），連帶把
   「safety net 改向但內文 P&F 句殘留原方向」的舊縫隙一起堵掉

### 驗證
- pytest 496/496（487 + 9 新 `test_pnf_direction_enforce.py`）
- 撼訊 7/17 實句 dry-run：short 引多方句 → 替換為空方句 ✓
- §5「空標：X 元（P&amp;F 下行目標）」不受誤傷（HTML entity 隔離 + 前綴不符）
