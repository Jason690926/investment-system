...
def analyze_stock_three_masters(
    ...
    _pnf = calc_pnf_target(enriched_data.get('weekly_bars', []), lookback=12)
    if not _pnf:
        _pnf = calc_pnf_target(enriched_data.get('daily_bars', []), lookback=20)
    pnf_str = f'{_pnf:.0f}' if _pnf else '—'
