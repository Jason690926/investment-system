...
class AIGenerationError(Exception):
    """AI 生成失敗（Bug-H §三十五）— 由 generate_personal_recommendation 等個股級函式
    在 _generate 回 error 字串時 raise，呼叫端可 catch 提供友善訊息 + 不污染 cache。

    _generate 本身保留 return error string 行為（向後相容 analyze_market_only 等
    其他批次/大盤 caller）。
    """
    def __init__(self, raw_error: str, kind: str = 'generic'):
        self.raw_error = raw_error
        self.kind = kind  # 'credit' / 'rate_limit' / 'timeout' / 'generic'
        super().__init__(raw_error)

    @property
    def friendly_message(self) -> str:
        if self.kind == 'credit':
            return 'AI 服務額度不足，請聯絡管理員充值（不寫入個人建議快取）'
        if self.kind == 'rate_limit':
            return 'AI 服務暫時繁忙，請稍候再試'
        if self.kind == 'timeout':
            return 'AI 服務回應逾時，請重試'
        snippet = (self.raw_error or '')[:80]
        return f'AI 分析暫時無法使用：{snippet}'


def _detect_error_kind(response: str) -> str:
    """偵測 _generate 回的字串種類。Bug-H §三十五。

    回傳：'ok' / 'credit' / 'rate_limit' / 'timeout' / 'generic'
    'ok' 表示為正常 AI response（非 'AI分析失敗' 開頭）；其他為失敗種類。
    """
    if not isinstance(response, str) or not response.startswith('AI分析失敗'):
        return 'ok'
    low = response.lower()
    if 'credit balance' in low or 'invalid_request_error' in low:
        return 'credit'
    if 'rate_limit' in low or '429' in response:
        return 'rate_limit'
...
def _dual_swing_block(enriched_data: dict, price_f) -> str:
    ...
    # R6 §四十三：錨點一致性鐵律 — 修「§1/§3 敘事價位 vs §5 錨點不同步」
    # （華星光 §1/§3 等回測 454 vs §5 空進 471.5~489、矽力同節兩壓力區 7/16 案例）
    lines.append(
...
def _structure_block(enriched_data: dict, price_f) -> str:
    ...
    return (
        ...
        f" ｜ 守穩支撐：{hold}\n"
        "⚠️ 內文提及「距峰值回落」「連續月陰線」等須直接引用上列程式數值，"
        "禁止改用盤中高點/收盤等其他基準自行算出不同數字。"
    )
...
⚠️ **客觀性鐵律（Bug-G §三十五）**：RISK_PCT 是**純技術面評分**（K 線 / 結構 / 趨勢 / 量價），**禁止考慮「讀者是否持股」「個人成本/損益」**等個人化因素。家人讀者可能是觀察方、可能是持有方 — 同一支股 RISK 數值必須一致。即使收到【持倉提示】也只影響「第六節是否輸出」，**不可影響 RISK 評分**。
...
def analyze_market_only(
    ...
    # Bug-G §三十五：明確隔離影響範圍 — 避免污染 RISK_PCT/方向/量價評估
    if is_holding:
        _structure_block_text += (
            '\n\n【持倉提示】請務必輸出第六節「持倉部位建議」（user-agnostic 模板，'
            '不含個人成本/損益）。⚠️ 此提示**僅影響第六節是否輸出**，'
            '禁止影響 RISK_PCT 評分、WYCKOFF_PHASE 判定、DIRECTION 方向、量價分析。'
            'RISK 必須是純技術面客觀評估，同一支股對「持股人」「觀察者」「觀望者」'
            '三類讀者數值必須一致。'
        )
...
⚠️ 不構成實際投資建議，投資人需自行評估風險。"""
    ...
    # Bug-H §三十五：失敗時 raise 而非回 error 字串，避免污染 PersonalRecommendation cache
    raw = _generate(prompt, max_tokens=1200)
    kind = _detect_error_kind(raw)
    if kind != 'ok':
        raise AIGenerationError(raw_error=raw, kind=kind)
    return raw
