...
def generate_daily_report(global_markets, commodities, news,
    ...
    for name, data in global_markets.items():
        ...
        markets_html += f'<tr><td>{name}</td><td class="close-price">{data["price"]:,}</td><td class="{cls}">{sign}{data["change"]}%</td></tr>'

    macro_html = ''
    for name, data in macro_data.items():
        cls = 'up' if data['change'] > 0 else 'down'
        sign = '+' if data['change'] > 0 else ''
        macro_html += f'<tr><td>{name}</td><td class="close-price">{data["price"]}</td><td class="{cls}">{sign}{data["change"]}%</td></tr>'
...
<div class="stock-card">
  <div class="stock-header">
    <div class="stock-title">{name} <span style="font-size:12px;color:#888;font-weight:400">({symbol})</span></div>
  ...
  <div class="pattern-row">K線型態：{pattern_html}</div>
  ...
  <div class="ai-advice">{_md(ai_advice)}</div>
</div>'''
    else:
        watchlist_html = '<div style="color:#888;padding:16px;text-align:center">尚未設定追蹤標的，請至控制台新增</div>'
    ...
    if watchlist_analysis:
        ...
        for w_item in watchlist_analysis:
            ...
            if w_name not in all_signals:
                w_sig = check_entry_signal(w_name, w_item['technical'])
                all_signals[w_name] = w_sig
    ...
    for name, sig in all_signals.items():
        ...
        watchlist_mark = ' ⭐' if is_watchlist else ''
        signals_html += f'<tr{row_style}><td>{name}{watchlist_mark}</td><td>{sig["recommendation"]}</td><td>{sig["score"]}</td><td class="{cls}">{sig["action"]}</td><td>{passed_text}</td></tr>'
...
.neutral{color:#f39c12;font-weight:500}
...
.analysis-text ul{margin:4px 0;padding-left:20px}
.analysis-text li{margin:3px 0}
...
.stock-card{background:#f8f9ff;border:1px solid #c5cae9;border-radius:8px;padding:14px;margin-bottom:12px}
.stock-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:8px}
.stock-title{font-size:15px;font-weight:bold;color:#1a237e}
...
.bullish-tag{background:#e8f5e9;color:#2e7d32}
.bearish-tag{background:#ffebee;color:#c62828}
.neutral-tag{background:#f5f5f5;color:#666}
...
.ai-advice p{margin:5px 0}
.ai-advice ul{padding-left:20px;margin:4px 0}
.ai-advice li{margin:3px 0}
...
<div class="section">
  <h2>李佛摩法則訊號總覽</h2>
  <table>
    <tr><th>股票</th><th>建議</th><th>分數</th><th>動作</th><th>進場區間</th><th>目標價</th><th>停損價</th><th>通過條件</th></tr>
    {signals_html}
  </table>
</div>

<div class="disclaimer">
免責聲明：本報表為自動化系統產生，所有分析與建議僅供學習參考，不構成實際投資建議。投資有風險，請自行評估後謹慎決策。
</div>
</body></html>'''
...
<div class="stock-card">
  ...
  <div class="stock-meta">趨勢：{tech.get("trend","--")} ｜ RSI：{tech.get("RSI","--")} ｜ MA5：{tech.get("MA5","--")} ｜ MA20：{tech.get("MA20","--")}{(" ｜ " + pnl_html) if pnl_html else ""}</div>
  <div class="pattern-row">本週K線型態：{pattern_html}</div>
  <div class="price-summary">
    <span class="support-level">支撐：{tech.get("support","--")}</span>
    <span class="resistance-level">壓力：{tech.get("resistance","--")}</span>
    <span class="close-price">進場：{tech.get("entry_low","--")}~{tech.get("entry_high","--")}</span>
  ...
  <div class="ai-advice">{_md(ai_advice)}</div>
        ...
        watchlist_html = '<div style="color:#888;padding:16px;text-align:center">尚未設定追蹤標的</div>'
    ...
    for name, sig in all_signals.items():
        ...
        signals_html += f'<tr{row_style}><td>{name}{watchlist_mark}</td><td>{sig["recommendation"]}</td><td>{sig["score"]}</td><td class="{cls}">{sig["action"]}</td><td>{entry}</td><td>{target}</td><td>{stop}</td><td>{passed_text}</td></tr>'

    css = '''
body{font-family:"Microsoft JhengHei",Arial,sans-serif;margin:0;padding:20px;color:#333;font-size:14px;background:#f5f5f5}
.header{background:#1b5e20;color:white;padding:24px;border-radius:8px;margin-bottom:20px}
.header h1{margin:0 0 6px 0;font-size:22px}
.header p{margin:0;opacity:.85;font-size:13px}
.week-badge{display:inline-block;background:rgba(255,255,255,0.2);padding:4px 12px;border-radius:20px;font-size:12px;margin-top:8px}
.section{background:white;border:1px solid #e0e0e0;border-radius:8px;padding:16px;margin-bottom:16px}
.section h2{margin:0 0 14px 0;font-size:16px;color:#1b5e20;border-bottom:2px solid #1b5e20;padding-bottom:6px}
table{width:100%;border-collapse:collapse;font-size:13px}
th{background:#e8f5e9;padding:8px;text-align:left;font-weight:600}
td{padding:7px 8px;border-bottom:1px solid #f0f0f0}
.up{color:#e74c3c;font-weight:500}
.down{color:#27ae60;font-weight:500}
.neutral{color:#f39c12;font-weight:500}
...
.analysis-text ul{margin:4px 0;padding-left:20px}
.analysis-text li{margin:3px 0}
.analysis-text h1,.analysis-text h2,.analysis-text h3{margin:12px 0 6px 0;font-size:14px;color:#1b5e20}
.short-term-title{display:block;color:#d84315;font-weight:700;font-size:14px;border-left:4px solid #d84315;padding:6px 0 6px 10px;margin:14px 0 6px 0;background:#fff3e0;border-radius:0 4px 4px 0}
.mid-term-title{display:block;color:#1565c0;font-weight:700;font-size:14px;border-left:4px solid #1565c0;padding:6px 0 6px 10px;margin:14px 0 6px 0;background:#e3f2fd;border-radius:0 4px 4px 0}
.target-price{color:#2e7d32;font-weight:700;background:#e8f5e9;padding:1px 6px;border-radius:4px}
.stop-loss{color:#b71c1c;font-weight:700;background:#ffebee;padding:1px 6px;border-radius:4px}
.support-level{color:#1565c0;font-weight:700;background:#e3f2fd;padding:1px 6px;border-radius:4px}
.resistance-level{color:#c62828;font-weight:700;background:#fff0f0;padding:1px 6px;border-radius:4px}
.stock-card{background:#f1f8e9;border:1px solid #a5d6a7;border-radius:8px;padding:14px;margin-bottom:12px}
.stock-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:8px}
.stock-title{font-size:15px;font-weight:bold;color:#1b5e20}
.stock-price{font-size:15px;font-weight:bold}
.stock-meta{font-size:12px;color:#666;margin-bottom:8px;line-height:1.7}
.pattern-row{margin-bottom:8px}
.price-summary{display:flex;flex-wrap:wrap;gap:8px;margin-bottom:10px;padding:8px;background:#f5f5f5;border-radius:6px}
...
.bearish-tag{background:#ffebee;color:#c62828}
.neutral-tag{background:#f5f5f5;color:#666}
.ai-advice{font-size:13px;color:#444;line-height:1.8;background:white;border-radius:6px;padding:12px;border-left:3px solid #1b5e20}
.ai-advice p{margin:5px 0}
.ai-advice ul{padding-left:20px;margin:4px 0}
.ai-advice li{margin:3px 0}
.ai-advice h1,.ai-advice h2,.ai-advice h3{font-size:13px;margin:8px 0 4px 0;color:#1b5e20}
.disclaimer{background:#fff3e0;border:1px solid #ffe0b2;border-radius:6px;padding:12px;font-size:12px;color:#e65100;margin-top:16px}
'''
